...
type
  Tfrmjagt = class(TForm)
    ...
    EditUndo1: TEditUndo;
    ExcelIO1Alternativas: TAdvGridExcelIO;
    N5: TMenuItem;
    Exportaraexcel1: TMenuItem;
    Importaralternativas2: TMenuItem;
    Importarcomplementos2: TMenuItem;
   ...
   procedure FormCreate(Sender: TObject);
    ...
    Function axv_getArticulo(grd:TAdvStringGrid; columna,fila:integer; nombre,clave,art_id:string):boolean;
    ...
    procedure Exportar_a_excelExecute(Sender: TObject);
    procedure Importar_complementosExecute(Sender: TObject);
    procedure Importar_alternativasExecute(Sender: TObject);
...
Function Tfrmjagt.axv_getArticulo(grd:TAdvStringGrid;columna,fila:integer; nombre,clave,art_id:string):Boolean;
var
  fqCargar:TdmQuerys;
  C1,N1:String;
  Repetido:Boolean;
...
begin
  Result:=false;
  Repetido:=False;
  C1:=grd.Cells[cClave,Fila];
  N1:=grd.Cells[cNombre,Fila];
  fqCargar:=TdmQuerys.Create(nil);
  fqCargar.dbtTransaccion.DefaultDatabase:=dbConectar.idbDatabase;
  fqCargar.figQuery.Database:=dbConectar.idbDatabase;
  fqCargar.dbtTransaccion.Active:=true;
  with fqCargar.figQuery do begin
    ...
    IF fn('articulo_id').AsString='' then begin
      MessageDlg('El articulo no se encuentra registrado' +#13#10
        + 'puede localizar el articul por medio de la clave' +#13#10
        + 'o el nombre. Para buscarlo oprima F4.',mtWarning,[mbOK],0);
      grd.Cells[cClave,Fila]:=C1;
      grd.Cells[cNombre,fila]:=N1;
    end else begin
      for i:=1 to grd.RowCount-1 do if (i <> fila) and (grd.Cells[cArticulo_id,i]=fn('articulo_id').AsString) then Repetido:=True;
      if Repetido then MessageDlg('Este articulo ya esta registrado',mtInformation,[mbOK],0)
      else begin
...
begin
  if ((aCol=cNombre) or (acol=cClave)) then begin //and (strgComplementos.Cells[cArticulo_id,aRow]='') then begin
    case ACol of
      cNombre: nombre:=Value;
      cClave: clave:=Value;
      cArticulo_id:art_id:=Value;
    end;
    Valid:=axv_getArticulo(strgComplementos,ACol,Arow,nombre,clave,art_id);
  end;
end;
...
var
  nombre,clave,art_id:string;
begin
  if ((aCol=cNombre) or (acol=cClave))then begin// and (sstrgAlternativas.Cells[cArticulo_id,aRow]='') then begin
    case ACol of
      cNombre: nombre:=Value;
      cClave: clave:=Value;
      cArticulo_id:art_id:=Value;
    end;
    Valid:=axv_getArticulo(sstrgAlternativas,ACol,Arow,nombre,clave,art_id);
  end;
end;
...
begin
  if not Modificar.Enabled then begin
    ...
    Importar_complementos.Enabled:=False;
...
begin
  ...
  if((C = cClave) or (C = cNombre)) then begin
    ...
    frmBuscarCliente.recupera_resultados:=False;
    ...
    if frmBuscarCliente.recupera_resultados then begin
      for i:=1 to grd.RowCount-1 do if (i <> R) and (grd.Cells[cArticulo_id,i]=frmBuscarCliente.resultadoID) then Repetido:=True;
      if Repetido then MessageDlg('Este articulo ya esta registrado',mtInformation,[mbOK],0)
      else begin
...
//    InputBox('','',text_consulta);
    ...
    begin
      ...
      edtClave.Text:=articulo_anterior;
      edtClave.SetFocus;
...
//    InputBox('','',text_consulta);
    ...
    axv_CargarComplementos(articulo_id);
    axv_CargarAlternativas(articulo_id);
  end else edtNombre.Text:= nombre_anterior;
end;

procedure Tfrmjagt.NuevoExecute(Sender: TObject);
begin
  if modificado then //preguntar y determinar que hacer con la respuesta
    case MessageDlg('Se modificaron las relaciones.' + #13#10 + '¿Desea guardar los cambios?',mtConfirmation,[mbYes,mbNo],0) of
      mrYes: Guardar.Execute;
    end;
...
//  showmessage('Selected Cell ' + sstrgAlternativas.GridCells[Acol,aRow]);
  alternativa_anterior := sstrgAlternativas.GridCells[Acol,aRow] // guardar alternativa_anterior para compararla con lo que se escribe
end;

procedure Tfrmjagt.sstrgAlternativasEditCellDone(Sender: TObject; ACol,
  ARow: Integer);
begin
  if not modificado then modificado := alternativa_anterior <> sstrgAlternativas.GridCells[ACol, ARow];
  {if modificado then //évitar que se pueda guardar si no se ha cambiado ningun dato
  begin
...
procedure Tfrmjagt.strgComplementosEditCellDone(Sender: TObject; ACol,
  ARow: Integer);
begin
  if not modificado then modificado := complemento_anterior <> strgComplementos.GridCells[ACol, ARow];
  {if modificado then //évitar que se pueda guardar si no se ha cambiado ningun dato
  begin
...
procedure Tfrmjagt.Exportar_a_excelExecute(Sender: TObject);
var
  direccion:string;
begin
  OpenDialog1 := TOpenDialog.Create(self);
  OpenDialog1.InitialDir := GetCurrentDir;
  OpenDialog1.Filter:='Excel Worksheets|*.xlsx|*.xls';
  OpenDialog1.FileName:= 'Altr y cmpl de ' + edtClave.Text + '.xls';
  if openDialog1.Execute then begin
    direccion:=OpenDialog1.FileName;
    {Primero exportamos el grid alternativas si es que existe alguna}
    if (sstrgAlternativas.RowCount > 1) and (sstrgAlternativas.Cells[carticulo_id,sstrgAlternativas.RowCount-1]<>'')
    then with ExcelIO1Alternativas do begin
        GridStartRow:=1;
        GridStartCol:=0;
        XlsStartRow:=1;
        XlsStartCol:=1;
        XLSExport(direccion,'Alternativas');
    end;// with ALTERNATIVAS
    {luego exportamos el grid complementos si es que existe alguno}
    if (strgComplementos.RowCount > 1) and (strgComplementos.Cells[carticulo_id,strgComplementos.RowCount-1]<>'')
    then with ExcelIO1Complementos do begin
      GridStartRow:=1;
      GridStartCol:=0;
      XlsStartRow:=1;
      XlsStartCol:=1;
      XLSExport(direccion,'Complementos');
    end;//with COMPLEMENTOS
  end;//if OpenDialog1.excecute
  OpenDialog1.Free;
end;

procedure Tfrmjagt.Importar_complementosExecute(Sender: TObject);
var
  direccion:string;
  I:INTEGER;
begin
  ...
  then begin
    ...
    if openDialog1.Execute then begin
      direccion:=OpenDialog1.FileName;
      //ShowMessage(direccion);
      with ExcelIO1Complementos do begin
        GridStartRow:=1;
        GridStartCol:=0;
        XlsStartRow:=1;
        XlsStartCol:=1;
      ...
      For i:=0 to strgComplementos.ColumnHeaders.Count-1 do BEGIN
        strgComplementos.Cells[i,0]:= strgComplementos.ColumnHeaders[i];
      END;
      For i:=1 to strgComplementos.RowCount-1 do begin
        axv_getArticulo(strgComplementos,cClave,i,
          strgComplementos.Cells[cNombre,i],
          strgComplementos.Cells[cClave,i],
          strgComplementos.Cells[cArticulo_id,i])
      end;
    end;//if OpenDialog1.excecute
    OpenDialog1.Free;
  end;
...
procedure Tfrmjagt.Importar_alternativasExecute(Sender: TObject);
var
  direccion:string;
  i:integer;
begin
  ...
  then begin
    ...
    if openDialog1.Execute then begin
      direccion:=OpenDialog1.FileName;
      //ShowMessage(direccion);
      with ExcelIO1Alternativas do begin
        GridStartRow:=1;
        GridStartCol:=0;
        XlsStartRow:=1;
        XlsStartCol:=1;
      ...
      For i:=0 to sstrgAlternativas.ColumnHeaders.Count-1 do BEGIN
        sstrgAlternativas.Cells[i,0]:= sstrgAlternativas.ColumnHeaders[i];
      END;
      For i:=1 to strgComplementos.RowCount-1 do begin
        axv_getArticulo(sstrgAlternativas,cClave,i,
          sstrgAlternativas.Cells[cNombre,i],
          sstrgAlternativas.Cells[cClave,i],
          sstrgAlternativas.Cells[cArticulo_id,i])
      end;
    end;//if OpenDialog1.excecute
    OpenDialog1.Free;
  end;//if excecute
